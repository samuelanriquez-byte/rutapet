...
export default function LandingPage() {
  return (
    <main style={{ background: '#0A1A0C', minHeight: '100vh', color: '#F0FDF4' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 40px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <span style={{ fontSize: 22, fontWeight: 800, color: '#4ADE80', letterSpacing: -0.5 }}>🐾 CicloPet</span>
        <div style={{ display: 'flex', gap: 12 }}>
          <Link href="/login" style={{ color: '#A3E4B5', textDecoration: 'none', fontSize: 14, padding: '8px 16px' }}>
            Iniciar sesión
          </Link>
          <Link href="/register" style={{ background: '#16A34A', color: '#fff', textDecoration: 'none', fontSize: 14, fontWeight: 600, padding: '8px 20px', borderRadius: 8 }}>
            Empezar gratis
          </Link>
        </div>
      </nav>
      ...
      <section style={{ maxWidth: 800, margin: '0 auto', padding: '100px 24px 80px', textAlign: 'center' }}>
        <div style={{ display: 'inline-block', background: 'rgba(74,222,128,0.12)', border: '1px solid rgba(74,222,128,0.3)', borderRadius: 99, padding: '6px 16px', fontSize: 13, color: '#4ADE80', marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize: 'clamp(36px, 6vw, 64px)', fontWeight: 900, lineHeight: 1.1, letterSpacing: -2, marginBottom: 24 }}>
          ...
          <span style={{ color: '#4ADE80' }}>se quedan sin alimento</span>
        </h1>
        <p style={{ fontSize: 18, color: '#A3E4B5', lineHeight: 1.7, maxWidth: 560, margin: '0 auto 48px' }}>
        ...
        <Link href="/register" style={{ display: 'inline-block', background: '#16A34A', color: '#fff', textDecoration: 'none', fontSize: 16, fontWeight: 700, padding: '16px 40px', borderRadius: 12 }}>
          Probá 14 días gratis →
        </Link>
        <p style={{ color: '#4B7A5A', fontSize: 13, marginTop: 16 }}>Sin tarjeta de crédito. Sin compromiso.</p>
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 24 }}>
          {[
            ...
            <div key={step.n} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 28 }}>
              ...
              <div style={{ fontSize: 11, color: '#4ADE80', fontWeight: 700, marginBottom: 8, letterSpacing: 1 }}>PASO {step.n}</div>
              <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 8 }}>{step.title}</h3>
              <p style={{ fontSize: 13, color: '#A3E4B5', lineHeight: 1.6 }}>{step.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ maxWidth: 480, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
        ...
        <p style={{ color: '#A3E4B5', marginBottom: 48, fontSize: 16 }}>Sin sorpresas. Sin límite de clientes.</p>
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(74,222,128,0.3)', borderRadius: 20, padding: 40 }}>
          <div style={{ fontSize: 56, fontWeight: 900, color: '#4ADE80', letterSpacing: -2 }}>$35</div>
          <div style={{ color: '#A3E4B5', marginBottom: 32, fontSize: 14 }}>USD / mes · Clientes ilimitados · WhatsApp incluido</div>
          ...
          <Link href="/register" style={{ display: 'block', background: '#16A34A', color: '#fff', textDecoration: 'none', fontWeight: 700, padding: '14px 0', borderRadius: 10, fontSize: 15 }}>
            Empezar ahora →
          </Link>
        </div>
      </section>
      ...
      {/* Footer */}
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.08)', padding: '32px 40px', textAlign: 'center', color: '#4B7A5A', fontSize: 13 }}>
